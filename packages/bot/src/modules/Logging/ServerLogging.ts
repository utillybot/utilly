	...
	private async _guildUpdate(
		...
		if (newGuild.systemChannelID != oldGuild.systemChannelID)
			overviewData.push({
				...
				old: oldGuild.systemChannelID
					? newGuild.channels.get(oldGuild.systemChannelID)?.mention ??
					  'No System Messages'
					: 'No System Messages',
				new: newGuild.systemChannelID
					? newGuild.channels.get(newGuild.systemChannelID)?.mention ??
					  'No System Messages'
					: 'No System Messages',
			});
		...
		if (newGuild.defaultNotifications != oldGuild.defaultNotifications)
			overviewData.push({
				...
				old: DEFAULT_NOTIFICATION_CONSTANTS[oldGuild.defaultNotifications],
				new: DEFAULT_NOTIFICATION_CONSTANTS[newGuild.defaultNotifications],
			});
		...
		if (newGuild.verificationLevel != oldGuild.verificationLevel)
			moderationData.push({
				...
				old: VERIFICATION_LEVEL_CONSTANTS_SHORT[oldGuild.verificationLevel],
				new: VERIFICATION_LEVEL_CONSTANTS_SHORT[newGuild.verificationLevel],
			});
		if (newGuild.explicitContentFilter != oldGuild.explicitContentFilter)
			moderationData.push({
				name: 'Explicit Media Content Filter',
				old: EXPLICIT_LEVEL_CONSTANTS_SHORT[oldGuild.explicitContentFilter],
				new: EXPLICIT_LEVEL_CONSTANTS_SHORT[newGuild.explicitContentFilter],
			});
